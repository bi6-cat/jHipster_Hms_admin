...
type RestOf<T extends IAppointment | NewAppointment> = Omit<T, 'appointmentDate' | 'startTime' | 'endTime'> & {
...
export class AppointmentService {
  ...
  protected convertDateFromClient<T extends IAppointment | NewAppointment | PartialUpdateAppointment>(appointment: T): RestOf<T> {
    return {
      ...appointment,
      appointmentDate: appointment.appointmentDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restAppointment: RestAppointment): IAppointment {
    return {
      ...restAppointment,
      appointmentDate: restAppointment.appointmentDate ? dayjs(restAppointment.appointmentDate) : undefined,

...
import { sampleWithRequiredData } from '../prescription.test-samples';
...
describe('Prescription Management Component', () => {
  ...
  it('should call load all on init', () => {
    ...
    expect(comp.prescriptions()[0]).toEqual(expect.objectContaining({ id: 6642 }));
  });

  describe('trackId', () => {
    it('should forward to prescriptionService', () => {
      const entity = { id: 6642 };
      jest.spyOn(service, 'getPrescriptionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPrescriptionIdentifier).toHaveBeenCalledWith(entity);

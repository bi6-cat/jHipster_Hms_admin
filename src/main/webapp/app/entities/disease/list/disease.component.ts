...
import { DiseaseService, EntityArrayResponseType } from '../service/disease.service';
...
export class DiseaseComponent implements OnInit {
  ...
  protected readonly diseaseService = inject(DiseaseService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IDisease): number => this.diseaseService.getDiseaseIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.diseases.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IDisease[] | null): IDisease[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.diseaseService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
